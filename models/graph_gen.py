...
def multi_layer_downsampling(points_xyz,
    base_voxel_size,
    levels=[1],
    add_rnd3d=False,):
    """Downsample the points using base_voxel_size at different scales"""
    xmax, ymax, zmax = np.amax(
        points_xyz, axis=0)  # Maxima along the first axis
        # [[x, x, x]
        # [y,y,y],
        # [z,z,z]]
    xmin, ymin, zmin = np.amin(points_xyz, axis=0) # Minima along the first axis

    xyz_offset = np.asarray([[xmin, ymin, zmin]])
    xyz_zeros = np.asarray([0, 0, 0], dtype=np.float32)
    downsampled_list = [points_xyz]
    last_level = 0
    for level in levels: # [1] -> N
        if np.isclose(last_level, level):  # 1e-08
...
def multi_layer_downsampling_random(points_xyz,
  base_voxel_size, levels=[1],
    add_rnd3d=False):
...
def gen_disjointed_rnn_local_graph_v3(
    ...
    indices = nbrs.radius_neighbors(center_xyz, return_distance=False) # 2d array
    if num_neighbors > 0:
        if neighbors_downsample_method == 'random':
            ...
            # [[1, 2, 3, 4], 0
            #  [190, 29, 12, 3] , 1]        [0 , 1, 2, 3, ...]
    vertices_v = np.concatenate(indices)  # [1, 2, 3, 4 ,190, 29, 12, 3]
